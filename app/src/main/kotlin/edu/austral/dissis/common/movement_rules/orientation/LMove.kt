...
class LMove: MovementRule {
    override fun validate(movement: Movement, board: Board): ValidationResult{
        if((abs(movement.getFrom().getRow() - movement.getTo().getRow()) == 2 && abs(movement.getFrom().getColumn() - movement.getTo().getColumn()) == 1) || (abs(movement.getFrom().getRow() - movement.getTo().getRow()) == 1 && abs(movement.getFrom().getColumn() - movement.getTo().getColumn()) == 2)){
            return ValidResult()
        }
        return InvalidResult("Invalid movement: not L move")
    }
}